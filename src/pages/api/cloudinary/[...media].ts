...
export const ALL: APIRoute = async ({ request, params }) => {
    try {
        ...
        const message = error instanceof Error ? error.message : "Unknown Cloudinary media error";

        console.error("Cloudinary media route failed:", error);

        return new Response(JSON.stringify({ e: message, message }), {
            status: 500,
            headers: {
                "Content-Type": "application/json",
            },
